...
def test_soap_body_endpoint_exists():
    ...
    response = client.post(
        "/prices/soap-body",
        json={"gold_price": 1000.0, "copper_price": 100000, "price_date": "2026-04-17"},
    )
...
def test_soap_body_xml_structure():
    ...
    response = client.post(
        "/prices/soap-body",
        json={"gold_price": 1057.9, "copper_price": 103300, "price_date": "2026-04-17"},
    )
...
def test_soap_body_guid_format():
    ...
    # Verify SAP GUID format: 32 uppercase hex characters without hyphens
    guid_pattern = r"^[0-9A-F]{32}$"
    assert re.match(guid_pattern, guid) is not None
